...
Смотри поле **exec cwd** — это каталог, откуда запускается бэкенд. Дальше все команды выполняй **в этом каталоге**.

## 2. Обновлён ли код

Убедись, что в коде бэкенда (в той папке) есть маршрут **POST /api/certificate**. Если его нет — обнови бэкенд (git pull или загрузка файлов).
