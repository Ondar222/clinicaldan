...
// icon — ключ из ICON_PATHS (src/components/CategoryIcon.tsx), SVG в стиле Heroicons
...
export const SERVICE_SUBCATEGORIES: ServiceSubcategory[] = [
  // Гинекология
  {
    ...
    description:
      "УЗИ органов малого таза, фолликулометрия, скрининг беременности",
  },
  {
...
export function getServiceSubcategory(
  serviceName: string,
  serviceGroup?: string,
  direction?: string,
): ServiceSubcategory | null {
  const text =
    `${serviceName} ${serviceGroup || ""} ${direction || ""}`.toLowerCase();
...
export function groupServicesByCategory(services: any[], direction?: string) {
  ...
  services.forEach((service) => {
    ...
    const subcategory = getServiceSubcategory(
      service.name,
      service.group,
      direction,
    );
